...
impl<F: FieldExt, Tr: Transcript<F>> Layer<F> for MulGate<F, Tr> {
    ...
    fn prove_rounds(
        ...
        let first_message = self
            .init_phase_1(claim.get_point().clone())
        ...
        let mut sumcheck_rounds: Vec<Vec<F>> = std::iter::once(Ok(first_message))
            .chain((1..num_rounds_phase1).map(|round| {
                ...
                let eval = prove_round(round + self.num_copy_bits, challenge, phase_1_mles)
                    .unwrap()
                    .into_iter()
                    .map(|eval| eval * self.beta_scaled.unwrap_or(F::one()))
                    .collect_vec();
        ...
        if f_2.bookkeeping_table.len() == 1 {
            ...
            let u_challenges = challenges.clone();
            let first_message = self
            ...
            if self.rhs_num_vars > 0 {
                ...
                let sumcheck_rounds_y: Vec<Vec<F>> = std::iter::once(Ok(first_message))
                    .chain((1..num_rounds_phase2).map(|round| {
                        let challenge = transcript.get_challenge("Sumcheck challenge").unwrap();
                        challenges.push(challenge);
                        let eval = prove_round(round + self.num_copy_bits, challenge, phase_2_mles)
                            .unwrap()
                            .into_iter()
                            .map(|eval| eval * self.beta_scaled.unwrap_or(F::one()))
                            .collect_vec();
    ...
    fn verify_rounds(
        ...
        let beta_g = if claim.get_point().len() > 0 {
    ...
    fn get_claims(&self) -> Result<Vec<Claim<F>>, LayerError> {
        let mut claims: Vec<Claim<F>> = vec![];
...
impl<F: FieldExt, Tr: Transcript<F>> MulGate<F, Tr> {
    ...
    ///
    pub fn init_phase_1(&mut self, claim_challenge: Vec<F>) -> Result<Vec<F>, GateError> {
        // --- First compute the bookkeeping table for \beta(g, z) \in \{0, 1\}^{s_i} ---

        let beta_g = if claim_challenge.len() > 0 {
            BetaTable::new(claim_challenge).unwrap()
        } else {
            BetaTable {
                layer_claim_vars: vec![],
                table: DenseMle::new_from_raw(vec![F::one()], LayerId::Input(0), None).mle_ref(),
    ...
    pub fn init_phase_2(
        ...
        let beta_u = BetaTable::new(u_claim_challenge).unwrap();
    ...
    pub fn dummy_prove_rounds(
        &mut self,
        claim_challenges: Vec<F>,
        rng: &mut impl Rng,
    ) -> Result<Vec<(Vec<F>, Option<F>)>, GateError> {
        // initialization
        let first_message = self
            .init_phase_1(claim_challenges)
            .expect("could not evaluate original lhs and rhs");
        let phase_1_mles = self
            .phase_1_mles
            .as_mut()
        ...
        for round in 1..(num_rounds_phase1) {
            ...
            let eval = prove_round(round + self.num_copy_bits, chal, phase_1_mles).unwrap();
        ...
        if f_2.bookkeeping_table.len() == 1 {
            ...
            for round in 1..num_rounds_phase2 {
                ...
                let chal = challenge.unwrap();
                challenges.push(chal);
                let eval = prove_round(round + self.num_copy_bits, chal, phase_2_mles).unwrap();
                messages.push((eval, challenge));
            }
...
#[cfg(test)]
mod test {
    use crate::mle::{batched_mulgate::MulGateBatched, dense::DenseMle};
