...
pub struct BinDecomp64Bit<F> {
    ///The 64 bits that make up this decomposition
    ///
    /// Should all be 1 or 0
    // pub bits: [F; 64],
    // TODO!(ryancao): Is this bad??? We can't auto-derive Serialize for [F; 64] unfortunately -- only goes up to 32...
    pub bits: Vec<F>,
}

impl<F: FieldExt> From<Vec<bool>> for BinDecomp64Bit<F> {
...
// --- Bin decomp ---
impl<F: FieldExt> MleAble<F> for BinDecomp64Bit<F> {
    type Repr = [Vec<F>; 64];
    ...
    type IntoIter<'a> = std::vec::IntoIter<BinDecomp64Bit<F>> where Self: 'a;
    ...
    fn from_iter(iter: impl IntoIterator<Item = Self>) -> Self::Repr {
        ...
        let mut ret: [Vec<F>; 64] = std::array::from_fn(|_| vec![]);
        iter.for_each(|tuple| {
    ...
    fn num_vars(items: &Self::Repr) -> usize {
        log2(64 * items[0].len()) as usize
    }
}

// TODO!(ryancao): Make this stuff derivable
impl<F: FieldExt> DenseMle<F, BinDecomp64Bit<F>> {
    /// Returns a list of MLERefs, one for each bit
    /// TODO!(ryancao): Change this back to [DenseMleRef<F>; 64] and make it work!
        ...
        for bit_idx in 0..64 {
            ...
            .chain(
                std::iter::once(MleIndex::Fixed(first_prefix))
                    ...
                    .chain(std::iter::once(MleIndex::Fixed(sixth_prefix)))
                    .chain(repeat_n(MleIndex::Iterated, num_vars - 6)),
            )
            .collect_vec();
            ...
            DenseMle::new_from_raw(
                ...
                Some(
                    self.get_prefix_bits().iter().flatten().cloned().chain(
                        repeat_n(MleIndex::Iterated, 6)
                    ).collect_vec()
                )
            ).mle_ref()
        }).to_vec();
        combine_mles(self_mle_ref_vec, 6)
    }

    /// Combines the bookkeeping tables of each of the MleRefs within a
    /// `DenseMle<F, BinDecomp64Bit<F>>` into a single interleaved bookkeeping
    ...
    /// the `DenseMle<F, BinDecomp64Bit<F>>`.
    /// 
    /// TODO!(ende): refactor
    pub fn combine_mle_batch(input_mle_batch: Vec<DenseMle<F, BinDecomp64Bit<F>>>) -> DenseMle<F, F> {
