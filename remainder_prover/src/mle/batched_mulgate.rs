...
use crate::{
    ...
    mle::{
        ...
        dense::DenseMle,
        mulgate::check_fully_bound,
    },
    prover::SumcheckProof,
    sumcheck::{evaluate_at_a_point, Evals, MleError, VerifyError},
};

use super::{
    dense::DenseMleRef,
    mulgate::{compute_full_mulgate, fix_var_gate, GateError, MulGate},
    MleIndex, MleRef,
};
...
impl<F: FieldExt, Tr: Transcript<F>> Layer<F> for MulGateBatched<F, Tr> {
    ...
    fn get_claims(&self) -> Result<Vec<Claim<F>>, LayerError> {
        ...
        // TODO(Makis): Check to/from are correctly set.
        ...
        // TODO(Makis): Check that to/from are correctly set.
        claims.push(Claim::new(
            fixed_mle_indices_v,
            val,
            Some(self.layer_id),
...
impl<F: FieldExt, Tr: Transcript<F>> MulGateBatched<F, Tr> {
    ...
    fn dummy_prove_rounds(
        ...
        let next_claims = self.g1_challenges.clone().unwrap();
    ...
    fn dummy_verify_rounds(
        ...
        let f2_bound = claims[0].get_result();
        let f3_bound = claims[1].get_result();
...
fn compute_sumcheck_message_copy_phase_mul<F: FieldExt>(
    ...
    let eval =
        evaluate_mle_ref_product(mles, independent_variable, degree, beta.table.clone()).unwrap();
...
mod test {
    ...
    fn test_evals_please_work_for_batched_4() {
        ...
        let mut beta_g2: DenseMleRef<Fr> =
            DenseMle::new_from_raw(vec![Fr::from(0), Fr::from(1)], LayerId::Layer(203091), None)
                .mle_ref();

        let mut beta_g1: DenseMleRef<Fr> = DenseMle::new_from_raw(
            vec![Fr::from(1).neg(), Fr::from(2)],
            LayerId::Layer(203091),
            None,
        )
        .mle_ref();
    ...
    fn test_sumcheck_batched_4() {
        ...
        let claim = Claim::new_raw(vec![Fr::from(1), Fr::from(2)], Fr::from(12));
