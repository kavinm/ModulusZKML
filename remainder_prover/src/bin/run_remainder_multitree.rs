...
use remainder::{
    ...
    zkdt::{
        ...
        }, zkdt_multiple_tree_circuit::ZKDTMultiTreeCircuit, structs::ZKDTProof,
    },
};
        ...
        Ok(proof) => {
            ...
            if verify_proof {
                ...
                let proof = if let Some(filepath_to_proof) = maybe_filepath_to_proof {
                    ...
                    let zkdt_proof: ZKDTProof<F, C::Transcript> = serde_json::de::from_slice(&bufreader[..]).unwrap();
...
fn main() -> Result<(), ZKDTBinaryError> {
    ...
    let (trees_batched_data, (tree_height, input_len), minibatch_data) = load_upshot_data_multi_tree_batch::<Fr>(maybe_minibatch_data.clone(), args.tree_batch_size, args.tree_batch_number, Path::new(&args.decision_forest_model_filepath), Path::new(&args.quantized_samples_filepath));
    ...
    run_zkdt_circuit(full_zkdt_circuit, maybe_proof_filepath, args.verify_proof, args.tree_batch_size)
}
