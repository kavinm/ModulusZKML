...
use crate::{
    ...
    mle::{beta::BetaTable, dense::DenseMle, mle_enum::MleEnum, Mle, MleIndex, MleRef},
    prover::{input_layer::InputLayer, GKRCircuit, Layers, Witness},
    sumcheck::{compute_sumcheck_message, get_round_degree, Evals},
};
...
impl<F: FieldExt> GKRCircuit<F> for PermutationCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let input_packing_builder = BatchedLayer::new(
            self.dummy_input_data_mle_vec
                .iter()
                .map(|input_data_mle| {
                    let mut input_data_mle = input_data_mle.clone();
                    input_data_mle.add_prefix_bits(Some(
                        repeat_n(MleIndex::Iterated, batch_bits).collect_vec(),
                    ));
...
impl<F: FieldExt> GKRCircuit<F> for MultiSetCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let decision_packing_builder = DecisionPackingBuilder::new(
            self.dummy_decision_nodes_mle.clone(),
            self.r,
            self.r_packings,
        );

        let leaf_packing_builder =
            LeafPackingBuilder::new(self.dummy_leaf_nodes_mle.clone(), self.r, self.r_packings.0);
        ...
        // layer 1
        let decision_leaf_concat_builder = ConcatBuilder::new(decision_packed, leaf_packed);
        let x_packed = layers.add_gkr(decision_leaf_concat_builder);

        // layer 2
        let r_minus_x_builder = RMinusXBuilder::new(x_packed, self.r);
        ...
        for i in 1..16 {
            // layer 3, or i + 2
            let r_minus_x_square_builder = SquaringBuilder::new(r_minus_x);
            ...
            // layer 5, or i + 4
            let prod_builder = ProductBuilder::new(curr_prod, prev_prod);
        ...
        for _ in 0..self.tree_height {
            // layer 20, or i+20
            let prod_builder = SplitProductBuilder::new(exponentiated_nodes);
        ...
        // layer 1: concat
        let path_decision_leaf_concat_builder =
            ConcatBuilder::new(decision_path_packed, leaf_path_packed);
        let x_path_packed = layers.add_gkr(path_decision_leaf_concat_builder);

        // layer 2: r - x
        let r_minus_x_path_builder = RMinusXBuilder::new(x_path_packed, self.r);
        ...
        for _ in 0..log2(self.tree_height * self.num_inputs) {
            let prod_builder = SplitProductBuilder::new(path_product);
...
mod tests {
    ...
    use super::{AttributeConsistencyCircuit, PermutationCircuit};
    use crate::{
        prover::GKRCircuit,
        zkdt::zkdt_helpers::{
            generate_dummy_mles, generate_dummy_mles_batch, BatchedDummyMles, DummyMles,
            DUMMY_INPUT_LEN, NUM_DUMMY_INPUTS, TREE_HEIGHT,
        },
    };
    ...
    fn test_permutation_circuit() {
        ...
        match proof {
            Ok(proof) => {
                let mut transcript =
                    PoseidonTranscript::new("Permutation Circuit Verifier Transcript");
    ...
    fn test_attribute_consistency_circuit() {
        ...
        let mut transcript =
            PoseidonTranscript::new("Attribute Consistency Circuit Prover Transcript");
        ...
        match proof {
            Ok(proof) => {
                let mut transcript =
                    PoseidonTranscript::new("Attribute Consistency Circuit Verifier Transcript");
                let result = circuit.verify(&mut transcript, proof);
                if let Err(err) = result {
                    println!("{}", err);
                    panic!();
