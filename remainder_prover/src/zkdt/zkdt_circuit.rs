...
impl<F: FieldExt> ZKDTCircuit<F> {
    ...
    > {
        ...
        // --- a) Precommitted Ligero input layer for tree itself (LayerId: 0) ---
        let tree_mle_precommit_filepath = get_tree_commitment_filename_for_tree_number(
            0,
            Path::new("upshot_data/tree_ligero_commitments/"),
        );
...
/// The actual ZKDT circuit!
pub struct FakeCircuit<F: FieldExt> {
    /// All of the input MLEs coming from the data generation pipeline
    pub batched_catboost_mles: BatchedCatboostMles<F>,
}

impl<F: FieldExt> GKRCircuit<F> for FakeCircuit<F> {
    type Transcript = PoseidonTranscript<F>;
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        unimplemented!()
    }
    ...
    > {
        ...
        let attribute_consistency_witness = attribute_consistency_circuit.yield_sub_circuit();
        let attr_copy_witness = attribute_consistency_circuit.yield_sub_circuit();

        let (combined_circuit_layers, combined_circuit_output_layers) = combine_layers(
            vec![
                attribute_consistency_witness.layers,
                attr_copy_witness.layers,
            ],
            vec![
                attribute_consistency_witness.output_layers,
                attr_copy_witness.output_layers,
            ],
        )
        .unwrap();
...
impl<F: FieldExt> FakeCircuit<F> {
    fn create_sub_circuits(
        ...
        (
            AttributeConsistencyCircuit<F>,      
            Vec<InputLayerEnum<F, PoseidonTranscript<F>>>,
            Vec<CommitmentEnum<F>>        // input layers' commitments
        ),
        GKRError,
    > {
...
/// The actual ZKDT circuit!
pub struct BadCircuit<F: FieldExt> {
    /// All of the input MLEs coming from the data generation pipeline
    pub batched_catboost_mles: BatchedCatboostMles<F>,
}

impl<F: FieldExt> GKRCircuit<F> for BadCircuit<F> {
    type Transcript = PoseidonTranscript<F>;
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        unimplemented!()
    }
...
#[cfg(test)]
mod tests {
    use super::{ZKDTCircuit, FakeCircuit, BadCircuit};
    ...
    fn test_fake_circuit() {
        ...
        let combined_circuit = FakeCircuit {
            batched_catboost_mles,
        };
    ...
    fn test_bad_circuit() {
        ...
        let combined_circuit = BadCircuit {
            batched_catboost_mles,
        };
