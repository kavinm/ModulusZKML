...
pub struct InputLayer<F: FieldExt> {
    /// initially None; populated when `combine_input_mles` is called. represents the combined version of input mles.
...
mod tests {
    ...
    // use remainder_shared_types::FieldExt;
    use remainder_ligero::ligero_ml_helper::naive_eval_mle_at_challenge_point;
    ...
    fn simplest_test() {
        ...
        let mut dummy_input_layer = InputLayer::new_from_mles(&mut mle_list, None);
        ...
        let challenge_coord_1 = repeat_with(|| Fr::from(rng.gen::<u64>())).take(2).collect_vec();
        let challenge_coord_2 = repeat_with(|| Fr::from(rng.gen::<u64>())).take(1).collect_vec();
    ...
    fn simple_test() {
        ...
        // --- point and ensure they're the same ---
        // let mut rng = test_rng();
        // let challenge_coord_1 = repeat_with(|| Fr::from(rng.gen::<u64>())).take(5).collect_vec();
        // let challenge_coord_2 = repeat_with(|| Fr::from(rng.gen::<u64>())).take(5).collect_vec();
        // let challenge_coord_3 = repeat_with(|| Fr::from(rng.gen::<u64>())).take(4).collect_vec();
    ...
    #[test]
    #[ignore = "Test assumes big-endian but implementation is little-endian"]
    ...
    fn test_invert_mle_bookkeeping_table() {
        ...
        let random_challenge = repeat_with(|| Fr::from(rng.gen::<u64>())).take(num_vars).collect_vec();
