...
use crate::{
    ...
    layer::{layer_enum::LayerEnum, Layer, LayerId, GKRLayer, from_mle},
    mle::{MleIndex, MleRef, mle_enum::MleEnum}, utils::{bits_iter, argsort},
};
...
pub fn combine_layers<F: FieldExt, Tr: Transcript<F>>(
    ...
    let interpolated_layers = (0..layer_count)
    .map(|layer_idx| {
        layers
            .iter()
    ...
    let bit_counts: Vec<Vec<Vec<MleIndex<F>>>> = interpolated_layers.clone().map(|layers| {
        ...
        let extra_bits = layer_sizes.clone().map(|size| total_size - size).collect_vec();
        ...
        .sorted_by(|(index_first, _), (index_second, _)| {
            index_first.cmp(index_second)
        }).map(|(_, bits)| bits).collect_vec()
    }).filter(|item: &Vec<Vec<MleIndex<F>>>| item.len() > 1).collect_vec();

    //The layers of the circuit are the inner vec
    let layer_bits = (0..layers.len()).map(|index| bit_counts.iter().map(|bit_counts| bit_counts[index].clone()).collect_vec()).collect_vec();

    layers.iter_mut().zip(output_layers.iter_mut()).zip(layer_bits).map(|((layers, output_layers), new_bits)| {
        for (layer_idx, new_bits) in new_bits.into_iter().enumerate() {
            if let Some(&effected_layer) = layers.0.get(layer_idx).map(|layer| layer.id()) {
                add_bits_to_layer_refs(&mut layers.0[layer_idx..], output_layers, new_bits, effected_layer)?;
            }
        }
        Ok(())
    }).try_collect()?;
    ...
    .map(|layer_idx| {
        layers
            ...
            //     }).collect_vec()
            // };
            let expressions = layers.into_iter().map(|layer| {
                match layer {
    ...
    Ok((Layers(layers), output_layers.into_iter().flatten().collect()))
}
...
fn combine_expressions<F: FieldExt>(mut exprs: Vec<ExpressionStandard<F>>) -> ExpressionStandard<F> {
    loop {
        if exprs.len() == 1 {
        ...
        exprs.sort_by(|first, second| {
            first.get_expression_size(0).cmp(&second.get_expression_size(0))
        });
...
fn add_padding<F: FieldExt>(mut expr: ExpressionStandard<F>, num_padding: usize) -> ExpressionStandard<F> {
