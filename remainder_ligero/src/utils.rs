...
pub fn get_random_coeffs_for_multilinear_poly<F: FieldExt>(ml_num_vars: usize) -> Vec<F> {
    ...
    repeat_with(|| F::from(rng.gen::<u64>())).take(2_usize.pow(ml_num_vars as u32)).collect()
}
...
pub fn halo2_fft<F: Halo2FieldExt>(coeffs: Vec<F>, rho_inv: u8) -> Vec<F> {
    ...
    let evaluation_domain: EvaluationDomain<F> = EvaluationDomain::new(rho_inv as u32, log_num_coeffs as u32);
...
pub fn halo2_ifft<F: Halo2FieldExt>(evals: Vec<F>, rho_inv: u8) -> Vec<F> {
    ...
    polynomial_coeff_form.iter().skip(num_coeffs).for_each(|coeff| {
...
pub fn get_least_significant_bits_to_usize_little_endian(bytes: Vec<u8>, num_bits: usize) -> usize {
    ...
    bytes.into_iter().enumerate().fold(0_usize, |acc, (idx, byte)| {
...
mod test {
    ...
    fn test_get_least_significant_bits() {
        ...
        (0..100).for_each(|_| {
            ...
            let actual_result = get_least_significant_bits_to_usize_little_endian(value_le_bytes, num_cols);
