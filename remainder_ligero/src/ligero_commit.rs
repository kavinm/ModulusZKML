use crate::adapter::{LigeroProof, convert_lcpc_to_halo, LigeroClaim};
...
use crate::{LcProofAuxiliaryInfo, LcRoot, verify};
use ark_std::log2;
use remainder_shared_types::{FieldExt, transcript::{Transcript as RemainderTranscript, TranscriptError}};
...
) -> Result<(F, LigeroEvalProof<PoseidonSpongeHasher<F>, LigeroEncoding<F>, F>), TranscriptError> {
    // --- Auxiliaries ---
    let rho = 1. / (rho_inv as f64);
    let num_rows = 2_usize.pow(log_num_rows as u32);
    let orig_num_cols = 2_usize.pow(log_orig_num_cols as u32);
...
pub fn remainder_ligero_commit_prove<F: FieldExt>(
    ...
    let (num_rows, orig_num_cols, _) = get_ligero_matrix_dims(input_mle_bookkeeping_table.len(), rho_inv).unwrap();
...
{
    // --- Sanitycheck ---
    assert_eq!(aux.num_rows * aux.orig_num_cols, 2_usize.pow(challenge_coord.len() as u32));

    // --- Grab the inner/outer tensors from the challenge point ---
    let (inner_tensor, outer_tensor) = get_ml_inner_outer_tensors(challenge_coord, aux.num_rows, aux.orig_num_cols);
...
#[cfg(test)]
pub mod tests {
    use crate::{ligero_commit::{poseidon_ml_commit_prove, poseidon_ml_eval_prove}, adapter::convert_halo_to_lcpc, ligero_ml_helper::{naive_eval_mle_at_challenge_point, get_ml_inner_outer_tensors}, verify};
    ...
    use remainder_shared_types::transcript::{Transcript as RemainderTranscript, poseidon_transcript::PoseidonTranscript};

    use super::{remainder_ligero_commit_prove, remainder_ligero_eval_prove, remainder_ligero_verify};
    ...
    fn test_remainder_flow() {
        ...
        let h2_ligero_proof: crate::adapter::LigeroProof<Fr> = remainder_ligero_eval_prove(&ml_coeffs, &challenge_coord, &mut poseidon_transcript, aux.clone(), comm, root.clone());
        ...
        remainder_ligero_verify::<Fr>(&root, &ligero_eval_proof, aux, &mut verifier_poseidon_transcript, &challenge_coord, claimed_value);
    }
    ...
    fn test_commit_eval_proof() {
        ...
        let result = verify(&root.root, &outer_tensor, &inner_tensor, &proof, &enc, &mut poseidon_transcript_verifier).unwrap();
