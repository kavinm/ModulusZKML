...
///A type that is responsible for FS over the interative version of the protocol
pub trait Transcript<F: FieldExt> {
    ///Create an empty transcript
    fn new(label: &'static str) -> Self;

    ///Append a single field element to the transcript
    fn append_field_element(&mut self, label: &'static str, element: F) -> Result<(), TranscriptError>;

    ///Append a list of field elements to the transcript
    fn append_field_elements(&mut self, label: &'static str, elements: impl IntoIterator<Item = F>) -> Result<(), TranscriptError>;
    ...
    fn get_challenges(&mut self, label: &'static str, len: usize) -> Result<Vec<F>, TranscriptError>;
}