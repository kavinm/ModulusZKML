...
use super::{Layer, LayerError, Claim, claims::ClaimError, LayerId};
...
impl<F: FieldExt, Tr: Transcript<F>> Layer<F> for EmptyLayer<F, Tr> {
    ...
    fn prove_rounds(
        ...
        let eval = gather_combine_all_evals(&self.expr).map_err(|err| LayerError::ExpressionError(err))?;
    ...
    fn verify_rounds(
        ...
        if sumcheck_rounds[0][0] != claim.1 {
            return Err(LayerError::VerificationError(super::VerificationError::GKRClaimCheckFailed))
        }
